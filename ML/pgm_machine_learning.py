...
usagers = sqlContext.sql("select * from usagers_tab").toPandas()
vehicules= sqlContext.sql("select * from vehicules_tab").toPandas()
charac= sqlContext.sql("select * from caracteristiques_tab").toPandas()
lieux= sqlContext.sql("select * from lieux_tab").toPandas() """

vehicules= pd.read_csv('/filRouge/spark/master/clean/vehicules_clean.csv',dtype=str)
usagers=pd.read_csv('/filRouge/spark/master/clean/usagers_clean.csv',dtype=str)
charac=pd.read_csv('/filRouge/spark/master/clean/caracteristiques_clean.csv',dtype=str)
lieux=pd.read_csv('/filRouge/spark/master/clean/lieux_clean.csv',dtype=str)
...
#Sensibilité 
print("Sensibilité : " + str(metrics.recall_score(y_test,y_pred,pos_label='1')))


# #  =================Arbre Decision ============================


arbreFirst=DecisionTreeClassifier(min_samples_split=30,min_samples_leaf=10 ,max_leaf_nodes=10)
...
print("MATRICE DE CONFUSION : " + str(metrics.confusion_matrix(y_test,pred)))
...
print("Sensibilité :" + str(metrics.recall_score(y_test,pred,pos_label='0')))

#précision – 93/(8+93)
print("Précison :" + str(metrics.precision_score(y_test,pred,pos_label='1')))